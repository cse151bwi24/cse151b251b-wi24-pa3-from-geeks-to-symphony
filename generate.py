...
def generate_song(model, device, char_idx_map, max_len=1000, temp=0.8, prime_str='<start>', show_heatmap=False):
    ...
    heatmap = torch.Tensor([]).to(device)
...
#         activations = []
        for i in range(len(prime)):
            _, activation = model(prime[i])
            heatmap = torch.cat((heatmap, activation), dim=0) 
        ...
        '''
        sampled_char = None
        generate_length = max_len - len(prime)

        for i in range(generate_length):
            # Get the last character's index
            input_char = generated_song[-1]
            input_char = torch.squeeze(characters_to_tensor(input_char, char_idx_map))
#             print(f"Input char: {input_char.shape}")
            # Pass the character through the model
            output, activation = model(input_char)
            heatmap = torch.cat((heatmap, activation), dim=0) 

            # Apply temperature to restrain ramdomness in the note that the model is generating
            output_dist = nn.functional.softmax(output / temp, dim=0)

            # Sample a character from the distribution
            sampled_char_idx = torch.multinomial(output_dist, 1)
...
#         hi = "char"
        ...
        for i in range(0, 150, 10):
            generate_heatmap(generated_song, heatmap.cpu(), neuron_idx = i)
...
def generate_heatmap(generated_song, heatmap, neuron_idx=0):
    ...
    plt.title(f"Heatmap For Song RNN, Neuron ID: {neuron_idx}", fontsize = 50)
